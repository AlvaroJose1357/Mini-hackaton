...
const ListActivitiesForm = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await axios.get(`http://localhost:5000/api/readActivity/${data.proyecto_id}`);
  ...
  return (
    <div className="max-w-lg mx-auto p-6 bg-white rounded-lg shadow-lg">
      <h2 className="text-2xl font-semibold text-center mb-6">Lista de Actividades por Proyecto</h2>
      ...
      {/* Mostrar actividades */}
      {loading && <p className="text-center text-blue-500">Cargando actividades...</p>}
        ...
        <div className="mt-6">
          ...
          <ul className="mt-4">
            {actividades.map((actividad) => (
              <li key={actividad.id} className="border-b py-2">
