...
const ActivityForm = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const { fecha_entrega, ...restData } = data;

      const response = await axios.post('http://localhost:5000/api/createActivity', restData);
  ...
  return (
    <div className="max-w-lg mx-auto p-6 bg-white rounded-lg shadow-lg">
      <h2 className="text-2xl font-semibold text-center mb-6">Agregar Actividad</h2>

      {/* Formulario usando React Hook Form */}
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="mb-4">
          <label htmlFor="proyecto_id" className="block text-sm font-medium text-gray-700">ID del Proyecto:</label>
          <input
            id="proyecto_id"
            type="text"
            {...register("proyecto_id", { required: "El ID del proyecto es obligatorio" })}
        ...
        <div className="mb-4">
          <label htmlFor="nombre" className="block text-sm font-medium text-gray-700">Nombre de la Actividad:</label>
          <input
            id="nombre"
            type="text"
            {...register("nombre", { required: "El nombre de la actividad es obligatorio" })}
            className="w-full mt-1 px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
